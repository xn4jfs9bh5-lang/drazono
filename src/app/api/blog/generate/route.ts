...
export async function POST(req: Request) {
  ...
  try {
    ...
    const msg = await client.messages.create({
      ...
      messages: [{ role: 'user', content: `Écris un article "${articleType || 'guide'}" sur: ${subject}. ${keyword ? 'Mot-clé: ' + keyword + '.' : ''} Pays: ${country || 'Général'}. ${words} mots. ${generateFaq !== false ? 'Inclus FAQ.' : ''} ${generateTable !== false ? 'Inclus tableau comparatif.' : ''} Format JSON: {"title":"...","slug":"...","content":"markdown","meta_description":"...","keywords":["..."],"faq":[{"question":"...","answer":"..."}],"suggested_articles":["..."]}.` }],
    })

    const text = msg.content[0].type === 'text' ? msg.content[0].text : ''
    console.log('[blog] len:', text.length, 'start:', text.substring(0, 80))
    ...
    if (!result) return NextResponse.json({ error: 'Réponse IA invalide. Réessayez.' }, { status: 500 })
